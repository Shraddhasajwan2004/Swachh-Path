...
import { 
  Trash2, Plus, AlertTriangle, Activity, Cpu, Database, Search,
  Wifi, WifiOff, Settings, X, Save, Loader2, CheckCircle, Bell
} from 'lucide-react';
...
function buildAlerts(bins: any[]): BinAlert[] {
  const now = new Date().toLocaleTimeString();
  return bins.map((bin) => {
    const reasons: string[] = [];
    let severity: Severity | null = null;

    if (bin.wet > 80)  { reasons.push(`Wet compartment critically full (${bin.wet}%)`);  severity = 'critical'; }
    if (bin.dry > 80)  { reasons.push(`Dry compartment critically full (${bin.dry}%)`);   severity = 'critical'; }
    if (bin.wet > 65 && bin.wet <= 80) { reasons.push(`Wet level approaching threshold (${bin.wet}%)`); if (!severity) severity = 'warning'; }
    if (bin.dry > 65 && bin.dry <= 80) { reasons.push(`Dry level approaching threshold (${bin.dry}%)`); if (!severity) severity = 'warning'; }
    if (bin.status === 'offline')      { reasons.push('Device offline — sensor data unavailable');        if (!severity) severity = 'warning'; }

    if (!severity) return null;
    return { binId: bin.id.toString(), binName: bin.name, severity, reasons, wet: bin.wet, dry: bin.dry, timestamp: now };
  }).filter(Boolean) as BinAlert[];
}
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'bins' | 'hardware' | 'api' | 'alerts'>('overview');
  const [searchQuery, setSearchQuery]   = useState('');
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isAdding, setIsAdding]         = useState(false);
  const [dismissed, setDismissed]       = useState<Set<string>>(new Set());
  const [alertFilter, setAlertFilter]   = useState<'all' | 'critical' | 'warning'>('all');
  const [newBin, setNewBin] = useState({ name: '', binId: '', channelId: '', readApiKey: '', writeApiKey: '' });
  ...
  const binAlerts   = useMemo(() => buildAlerts(bins), [bins]);
  const alertCount  = binAlerts.length; // badge on tab

  const visibleAlerts = binAlerts.filter(
    a => !dismissed.has(a.binId) && (alertFilter === 'all' || a.severity === alertFilter)
  );

  const chartData = useMemo(() => ({
    labels: bins.map(b => b.name),
    datasets: [
      { label: t('bin.wet'), data: bins.map(b => b.wet), backgroundColor: 'rgba(0,243,255,0.5)', borderColor: 'rgba(0,243,255,1)', borderWidth: 1 },
      { label: t('bin.dry'), data: bins.map(b => b.dry), backgroundColor: 'rgba(255,0,255,0.5)',  borderColor: 'rgba(255,0,255,1)',  borderWidth: 1 },
    ]
  }), [bins, t]);

  const filteredBins = bins.filter(b =>
    b.name.toLowerCase().includes(searchQuery.toLowerCase()) || b.id.toString().includes(searchQuery)
  );
  ...
  const barColor = (val: number) =>
    val >= 80 ? 'bg-cyber-danger' : val >= 65 ? 'bg-cyber-warning' : 'bg-cyber-accent';

  const tabs = ['overview', 'bins', 'hardware', 'api', 'alerts'] as const;
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center space-x-2 bg-white/5 p-1 rounded-xl border border-white/10">
          {tabs.map((tab) => (
            <button
              ...
              className={`relative px-4 py-2 rounded-lg text-[10px] font-display font-bold uppercase tracking-widest transition-all ${
                activeTab === tab
                  ? 'bg-cyber-accent text-white shadow-[0_0_15px_rgba(0,243,255,0.3)]'
                  : 'text-slate-500 hover:text-white hover:bg-white/5'
              }`}
            >
              {tab === 'alerts' ? 'Alerts' : t(`admin.${tab}`)}

              {/* Red badge on alerts tab when alerts exist */}
              {tab === 'alerts' && alertCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-cyber-danger text-white text-[8px] font-bold flex items-center justify-center">
                  {alertCount}
                </span>
              )}
            </button>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            <div className="lg:col-span-2 glass-panel p-8 border-white/5">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-display font-bold text-white uppercase tracking-widest flex items-center">
                  <Activity className="w-5 h-5 mr-3 text-cyber-accent" />{t('admin.fillLevels')}
                </h2>
              </div>
            ...
            <div className="glass-panel p-8 border-white/5">
              <h2 className="text-lg font-display font-bold text-white uppercase tracking-widest flex items-center mb-8">
                <AlertTriangle className="w-5 h-5 mr-3 text-cyber-danger" />{t('admin.alerts')}
              </h2>
              <div className="space-y-4">
                {alerts.length > 0 ? alerts.map((alert, idx) => (
                  <div key={idx} className="p-4 bg-cyber-danger/5 border border-cyber-danger/20 rounded-xl flex items-start space-x-3">
                    ...
                    <p className="text-[10px] font-display text-cyber-danger uppercase tracking-widest leading-relaxed">{alert}</p>
                  </div>
                )) : (
        ...
        {/* BINS */}
        {activeTab === 'bins' && (
          <motion.div key="bins" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
            className="glass-panel border-white/5 overflow-hidden"
          >
            <div className="p-8 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="relative flex-1 max-w-md">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                <input type="text" placeholder="Search bins by name or ID..." value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)} className="cyber-input w-full pl-12" />
              </div>
              <button onClick={() => setIsAddModalOpen(true)}
                ...
                <Plus className="w-4 h-4 mr-2" />Add New Bin
              </button>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.02]">
                    {[t('admin.idLocation'), t('admin.status'), t('admin.wetPercent'), t('admin.dryPercent'), t('admin.usage'), 'Actions'].map(h => (
                      <th key={h} className="px-8 py-4 text-[10px] font-display font-bold text-slate-500 uppercase tracking-widest">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredBins.map((bin) => (
                    <tr key={bin.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center space-x-3">
                          <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden min-w-[60px]">
                            <div className={`h-full transition-all duration-1000 ${bin.wet > 80 ? 'bg-cyber-danger' : 'bg-cyber-accent'}`} style={{ width: `${bin.wet}%` }} />
                          </div>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center space-x-3">
                          <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden min-w-[60px]">
                            <div className={`h-full transition-all duration-1000 ${bin.dry > 80 ? 'bg-cyber-danger' : 'bg-cyber-accent'}`} style={{ width: `${bin.dry}%` }} />
                          </div>
                          <span className="text-xs font-mono text-white">{bin.dry}%</span>
                        </div>
                      </td>
                      <td className="px-8 py-6"><span className="text-xs font-mono text-white">{bin.usage}</span></td>
                      <td className="px-8 py-6">
                        <button onClick={() => removeBinConfig(bin.id.toString())}
        ...
        {/* HARDWARE */}
        {activeTab === 'hardware' && (
          <motion.div key="hardware" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
            className="grid grid-cols-1 lg:grid-cols-2 gap-8"
          >
            <div className="glass-panel p-8 border-white/5">
              <h2 className="text-lg font-display font-bold text-white uppercase tracking-widest flex items-center mb-8">
                <Cpu className="w-5 h-5 mr-3 text-cyber-accent" />{t('admin.hardwareArch')}
              </h2>
            ...
            <div className="glass-panel p-8 border-white/5">
              <h2 className="text-lg font-display font-bold text-white uppercase tracking-widest flex items-center mb-8">
                <Settings className="w-5 h-5 mr-3 text-cyber-warning" />{t('admin.limitations')}
              </h2>
              <ul className="space-y-4">
                {[1, 2, 3, 4].map((i) => (
                  <li key={i} className="flex items-start space-x-3 p-4 bg-white/5 rounded-xl border border-white/5">
                    <div className="w-5 h-5 rounded-full bg-cyber-warning/10 border border-cyber-warning/20 flex items-center justify-center text-[10px] font-bold text-cyber-warning flex-shrink-0">{i}</div>
                    <p className="text-xs text-slate-400 leading-relaxed">{t(`admin.limit${i}`)}</p>
                  </li>
                ))}
              </ul>
            </div>
        ...
        {activeTab === 'api' && (
          ...
          >
            <h2 className="text-lg font-display font-bold text-white uppercase tracking-widest flex items-center mb-8">
              <Settings className="w-5 h-5 mr-3 text-cyber-accent" />{t('admin.apiConfig')}
            </h2>
            <div className="space-y-6">
              <div className="p-6 bg-black/40 rounded-2xl border border-white/5 font-mono text-xs text-cyber-accent leading-relaxed">
                ...
                {['field1: usage_count','field2: dry_count','field3: wet_count','field4: distance_cm','field5: latitude','field6: longitude','field7: dry_percent','field8: wet_percent'].map(f => (
                  <p key={f} className="ml-4">"{f.split(':')[0]}": {f.split(':')[1].trim()},</p>
                ))}
                <p>&#125;</p>
              </div>
        ...
        {activeTab === 'alerts' && (
          ...
          >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { label: 'Critical Alerts', value: binAlerts.filter(a => a.severity === 'critical').length, color: 'text-cyber-danger' },
                { label: 'Warnings',        value: binAlerts.filter(a => a.severity === 'warning').length,  color: 'text-cyber-warning' },
                { label: 'Bins Affected',   value: new Set(binAlerts.map(a => a.binId)).size,               color: 'text-cyber-accent' },
                { label: 'All Clear',       value: bins.length - new Set(binAlerts.map(a => a.binId)).size, color: 'text-cyber-success' },
              ].map((s, i) => (
                <div key={i} className="glass-panel p-6 border-white/5">
                  <p className="text-[10px] font-display text-slate-500 uppercase tracking-widest mb-2">{s.label}</p>
              ...
              <AnimatePresence>
                {visibleAlerts.length === 0 ? (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    className="glass-panel p-16 border-white/5 text-center"
                  >
                    <CheckCircle className="w-10 h-10 text-cyber-success mx-auto mb-4 opacity-40" />
                    <p className="text-[10px] font-display text-slate-500 uppercase tracking-widest">
                      {alertFilter === 'all' ? 'No active alerts — all bins nominal' : `No ${alertFilter} alerts`}
                    </p>
                  </motion.div>
                ) : (
                  visibleAlerts.map((alert) => (
                    <motion.div
                      ...
                      className={`glass-panel p-6 flex gap-5 items-start border ${
                        alert.severity === 'critical'
                          ? 'border-l-4 border-l-cyber-danger border-cyber-danger/20'
                          : 'border-l-4 border-l-cyber-warning border-cyber-warning/20'
                      }`}
                    >
                      {/* Icon */}
                      <div className={`p-3 rounded-xl flex-shrink-0 ${
                        alert.severity === 'critical'
                          ...
                          : 'bg-cyber-warning/10 border border-cyber-warning/20'
                      }`}>
                        <AlertTriangle className={`w-5 h-5 ${alert.severity === 'critical' ? 'text-cyber-danger' : 'text-cyber-warning'}`} />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                          <span className="text-sm font-bold text-white">{alert.binName}</span>
                          <span className={`px-2 py-0.5 rounded text-[10px] font-display font-bold uppercase tracking-widest ${
                            alert.severity === 'critical'
                        ...
                        <div className="space-y-2 mb-3">
                          {[{ label: 'Wet', val: alert.wet }, { label: 'Dry', val: alert.dry }].map(({ label, val }) => (
                            <div key={label} className="flex items-center gap-3">
                              <span className="text-[10px] font-display text-slate-500 uppercase tracking-widest w-6">{label}</span>
                              <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                                <div className={`h-full transition-all duration-700 ${barColor(val)}`} style={{ width: `${val}%` }} />
                              </div>
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-mono text-slate-600">ID: {alert.binId}</span>
                          <span className="text-[10px] font-display text-slate-600 uppercase tracking-widest">Detected at {alert.timestamp}</span>
                        </div>
                      </div>

                      {/* Dismiss button */}
                      <button
                        onClick={() => setDismissed(prev => new Set(prev).add(alert.binId))}
                        className="p-2 text-slate-600 hover:text-white hover:bg-white/10 rounded-lg transition-all flex-shrink-0"
                      >
                        <X className="w-4 h-4" />
                      </button>
                    </motion.div>
              ...
              <form onSubmit={handleAddBin} className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <input required type="text" value={newBin.name}
                      onChange={(e) => setNewBin({ ...newBin, name: e.target.value })}
                      className="cyber-input w-full" placeholder="e.g. Connaught Place #1" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-display uppercase tracking-widest text-slate-500 ml-1">Internal ID</label>
                    <input required type="text" value={newBin.binId}
                      onChange={(e) => setNewBin({ ...newBin, binId: e.target.value })}
                      className="cyber-input w-full" placeholder="e.g. BIN-001" />
                  </div>
                </div>
                {[
                  { label: 'ThingSpeak Channel ID', key: 'channelId', ph: 'e.g. 2456789', req: true },
                  { label: 'Read API Key',           key: 'readApiKey', ph: 'Enter Read API Key', req: true },
                  { label: 'Write API Key (Optional for Commands)', key: 'writeApiKey', ph: 'Enter Write API Key', req: false },
                ].map(({ label, key, ph, req }) => (
                  <div key={key} className="space-y-2">
                    <label className="text-[10px] font-display uppercase tracking-widest text-slate-500 ml-1">{label}</label>
                    <input required={req} type="text" value={(newBin as any)[key]}
                      onChange={(e) => setNewBin({ ...newBin, [key]: e.target.value })}
                      className="cyber-input w-full" placeholder={ph} />
                  </div>
                ))}
                <button type="submit" disabled={isAdding}
                  className="w-full py-4 bg-cyber-accent hover:bg-cyber-accent/90 text-white rounded-xl font-display font-bold uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(0,243,255,0.2)] flex items-center justify-center disabled:opacity-50">
                  {isAdding ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Save className="w-4 h-4 mr-2" />Save Configuration</>}
                </button>
              </form>
            </motion.div>
          </div>
