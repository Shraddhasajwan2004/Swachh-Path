...
import {
  Navigation, Mic, MicOff, Trash2, MapPin, ChevronRight,
  AlertTriangle, Activity, Droplets, Shield, Locate,
  Hash, Ruler, BarChart3, FileWarning,
} from 'lucide-react';
...
export default function UserPanel() {
  const { bins, sendCommand } = useBins();
  const { t, language }       = useLanguage();
  const ble                   = useBluetooth();
  const { triggerReportForBin } = useCommunity();
  ...
  const [activeAlert,   setActiveAlert]   = useState<{ message: string; bin?: any } | null>(null);
  const lastAlertedBins = useRef<Set<string>>(new Set());

  // Full-bin alerts — includes "File Report" shortcut into CommunityPage
  useEffect(() => {
    const fullBins = bins.filter(bin => (bin.field7 || 0) > 80 || (bin.field8 || 0) > 80);
    fullBins.forEach(bin => {
      if (!lastAlertedBins.current.has(bin.id)) {
        setActiveAlert({ message: `${bin.name} is filled! Use other bins.`, bin });
        lastAlertedBins.current.add(bin.id);
        setTimeout(() => setActiveAlert(null), 6000);
      }
    });
    const fullIds = new Set(fullBins.map(b => b.id));
    lastAlertedBins.current.forEach(id => { if (!fullIds.has(id)) lastAlertedBins.current.delete(id); });
  }, [bins]);
  ...
  const findNearestBin = useCallback(() => {
    if (!userLocation || !bins.length) return;
    let minDist = Infinity, closest: any = null;
    bins.forEach(bin => {
      const d = Math.hypot(bin.location.lat - userLocation.lat, bin.location.lng - userLocation.lng);
  ...
  const handleVoiceCommand = () => {
    ...
    if (!SpeechRecognition) { setVoiceFeedback('Speech recognition not supported.'); return; }
    ...
    recognition.onerror  = (e: any) => { setIsListening(false); setVoiceFeedback(`Error: ${e.error}`); };
    ...
    recognition.onresult = async (event: any) => {
      ...
      if (isDry) {
        if (ble.status === 'connected') { await ble.sendBLECommand('dry open'); setVoiceFeedback('BLE: Opening Dry Bin'); }
        else if (selectedBin) { try { await sendCommand(selectedBin.id, 'dry_open'); setVoiceFeedback('Command sent: Open Dry'); } catch { setVoiceFeedback('Failed.'); } }
        else setVoiceFeedback('Select a bin first.');
      } else if (isWet) {
        if (ble.status === 'connected') { await ble.sendBLECommand('wet open'); setVoiceFeedback('BLE: Opening Wet Bin'); }
        else if (selectedBin) { try { await sendCommand(selectedBin.id, 'wet_open'); setVoiceFeedback('Command sent: Open Wet'); } catch { setVoiceFeedback('Failed.'); } }
        else setVoiceFeedback('Select a bin first.');
      } else if (isNearest) {
        findNearestBin(); setVoiceFeedback('Finding nearest bin...');
      } else {
        setVoiceFeedback("Not recognised. Try 'dry open', 'wet open', or 'find nearest bin'.");
      }
  ...
  const nearbyBins = useMemo(() => {
    ...
    return [...bins]
      .map(bin => ({ ...bin, distance: Math.hypot(bin.location.lat - userLocation.lat, bin.location.lng - userLocation.lng) }))
    ...
    <div className="h-[calc(100vh-12rem)] flex flex-col lg:flex-row gap-8 relative">
      ...
      <AnimatePresence>
        {activeAlert && (
          <motion.div
            ...
            className="absolute top-4 left-1/2 -translate-x-1/2 z-[100] bg-cyber-danger/90 backdrop-blur-md border border-cyber-danger/50 px-6 py-3 rounded-full shadow-[0_0_30px_rgba(255,0,60,0.3)] flex items-center gap-4"
          >
            <AlertTriangle className="w-5 h-5 text-white animate-pulse flex-shrink-0" />
            <span className="text-xs font-display font-bold text-white uppercase tracking-widest">{activeAlert.message}</span>
            {activeAlert.bin && (
              <button
                onClick={() => {
                  const bin = activeAlert.bin;
                  triggerReportForBin(
                    bin.binId || bin.id,
                    bin.name,
                    bin.location?.address || `${bin.location?.lat}, ${bin.location?.lng}`
                  );
                  setActiveAlert(null);
                }}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded-full text-white text-[10px] font-display font-bold uppercase tracking-widest transition-all flex-shrink-0"
              >
                <FileWarning className="w-3.5 h-3.5" />File Report
              </button>
            )}
          </motion.div>
        )}
      ...
      <div className="w-full lg:w-96 flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
          ...
          <div className="relative flex items-center justify-between mb-4">
            <h2 className="text-sm font-display font-bold text-white uppercase tracking-widest flex items-center">
              <Mic className="w-4 h-4 mr-2 text-cyber-accent" />{t('user.voice')}
            </h2>
        ...
        <AnimatePresence mode="wait">
          {selectedBin ? (
            <motion.div key={selectedBin.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={async () => {
                      if (ble.status === 'connected') await ble.sendBLECommand('dry open');
                      else await sendCommand(selectedBin.id, 'dry_open');
                    }}
                    className={`flex-1 py-3 rounded-xl text-[10px] font-display font-bold uppercase tracking-widest transition-all bg-cyber-warning/10 hover:bg-cyber-warning/20 text-cyber-warning border border-cyber-warning/30 ${ble.status === 'connected' ? 'shadow-[0_0_15px_rgba(255,184,0,0.15)]' : 'opacity-70'}`}
                  >
                  ...
                  <button
                    onClick={async () => {
                      if (ble.status === 'connected') await ble.sendBLECommand('wet open');
                      else await sendCommand(selectedBin.id, 'wet_open');
                    }}
                    className={`flex-1 py-3 rounded-xl text-[10px] font-display font-bold uppercase tracking-widest transition-all bg-cyber-accent/10 hover:bg-cyber-accent/20 text-cyber-accent border border-cyber-accent/30 ${ble.status === 'connected' ? 'shadow-[0_0_15px_rgba(0,243,255,0.15)]' : 'opacity-70'}`}
                  >
        ...
        <div className="space-y-4">
          ...
          {nearbyBins.map(bin => (
            <button key={bin.id} onClick={() => setSelectedBin(bin)}
              className={`w-full p-4 glass-panel border-white/5 hover:border-cyber-accent/30 transition-all text-left flex items-center justify-between group ${selectedBin?.id === bin.id ? 'border-cyber-accent/50 bg-cyber-accent/5' : ''}`}>
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${(bin as any).wet > 80 || (bin as any).dry > 80 ? 'bg-cyber-danger/10 text-cyber-danger' : 'bg-cyber-accent/10 text-cyber-accent'}`}>
                ...
                <div>
                  ...
                  <p className="text-[8px] font-display text-slate-500 uppercase tracking-widest">
                    {t(`status.${bin.status}`, bin.status)} · {Math.round((bin as any).distance * 100000) / 100} {t('user.kmAway', 'km away')}
                  </p>
                </div>
              </div>
      ...
      <div className="flex-1 relative rounded-3xl overflow-hidden border border-white/5 shadow-2xl">
        <APIProvider apiKey={(import.meta as any).env?.VITE_GOOGLE_MAPS_API_KEY || GOOGLE_MAPS_KEY}>
          ...
          >
            ...
            <Directions origin={userLocation} destination={selectedBin?.location ?? null} onRouteInfo={setRouteInfo} />
          </Map>
        </APIProvider>
        ...
        <div className="absolute top-6 right-6 flex flex-col gap-3">
          <div className="glass-panel p-2 border-white/10 flex flex-col gap-2">
            <button className="p-2 hover:bg-white/5 rounded-lg text-slate-400 hover:text-white transition-all"><Plus className="w-5 h-5" /></button>
            <div className="h-px bg-white/5 mx-2" />
            <button className="p-2 hover:bg-white/5 rounded-lg text-slate-400 hover:text-white transition-all"><Minus className="w-5 h-5" /></button>
          </div>
          <button onClick={() => setSelectedBin(null)} className="glass-panel p-3 border-white/10 text-slate-400 hover:text-cyber-accent transition-all">
